...
from beta_paths import default_cache_dir, default_dataset_dir, resolve_repo_path, save_result
...
RESULT_PREFIX = "llama60m_slimpajama6b"
...
class PreprocessedIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        if worker_info is None:
            ...
            iter_data = itertools.islice(iter(self.data), worker_info.id, None, worker_info.num_workers)
...
def local_download_config() -> DownloadConfig:
    return DownloadConfig(local_files_only=True)


def load_split(dataset_cache_dir: Path, split: str):
...
def prepare_val_iterator(tokenizer, dataset_cache_dir: Path, eval_batch_size: int, max_length: int):
    ...
    val_data = load_split(dataset_cache_dir, "validation").shuffle(seed=42)
...
def build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Llama60M on local SlimPajama-6B with beta sweep support.")
...
def main() -> None:
    ...
    hf_cache_root = resolve_repo_path(
        args.hf_cache_dir,
        default_cache_dir("huggingface") / "llama60m_slimpajama6b",
    )
    dataset_cache_dir = resolve_repo_path(args.dataset_cache_dir, default_dataset_dir("slimpajama"))
    ...
    log(f"Run start | dataset=SlimPajama-6B beta={args.beta} seed={args.seed}")
    log(
    ...
    log(
        f"Paths | dataset_cache_dir={dataset_cache_dir} hf_cache_root={hf_cache_root} "
        f"run_cache_dir={run_cache_dir} model_config_path={model_config_path}"
    )
    ...
    train_data = load_split(dataset_cache_dir, "train").shuffle(seed=42)
