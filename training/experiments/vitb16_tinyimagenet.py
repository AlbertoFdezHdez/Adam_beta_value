...
from tinyimagenet_data import resolve_tinyimagenet_root
...
def main():
    ...
    dataset_root = resolve_tinyimagenet_root(args.data_root)
    ...
    train_set = datasets.ImageFolder(dataset_root / "train", transform=tf_train)
    val_set = datasets.ImageFolder(dataset_root / "val", transform=tf_val)
    ...
    if args.probe_init_batch_loss:
        out_path = run_classification_probe(
            args=args,
            prefix="vitb16_tinyimagenet",
            case_id="vitb16_tinyimagenet",
            label="ViT-B/16 + TinyImageNet",
            split="train",
            family="vision",
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if param.ndim == 1 or name.endswith(".bias") or "ln" in name.lower() or "norm" in name.lower():
    ...
    print("TinyImageNet root:", dataset_root)
