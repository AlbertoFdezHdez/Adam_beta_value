...
from caltech256_data import build_caltech256_datasets
...
def main() -> None:
    ...
    parser.add_argument("--min_lr", type=float, default=4e-6)
    ...
    data_root, train_set, val_set, num_classes, split_meta = build_caltech256_datasets(
        args.data_root,
        tf_train,
        tf_val,
    ...
    if args.probe_init_batch_loss:
        out_path = run_classification_probe(
            args=args,
            prefix="swin_t_caltech256",
            case_id="swin_t_caltech256",
            label="Swin-T + Caltech-256",
            split="val",
            family="vision",
            ...
            meta={
                ...
                "source_mode": split_meta.get("source_mode"),
            },
        )
    ...
    global_step = 0
    t_start = time.time()
    print("Caltech-256 categories root:", data_root)
    print("Split metadata:", {k: v for k, v in split_meta.items() if k != "class_names"})
    ...
    out_path = save_result("swin_t_caltech256", args, payload)
